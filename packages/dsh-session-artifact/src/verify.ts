...
import { SURFACE_ELIGIBLE_TYPES } from './events.ts';
 ...
 */
export function parseArtifact(jsonl: string): { header: SessionHeader; rows: SessionRow[] } | Violation {
  ...
  return { header: header as unknown as SessionHeader, rows };
}
 ...
 */
export function verifyArtifact(jsonl: string, root?: string): VerificationReport {
 ...
 */
const KNOWN_DSH_TYPES: ReadonlySet<string> = new Set([
  'turn/start',
  'turn/end',
  'step/start',
  'step/end',
